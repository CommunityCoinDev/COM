    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %sCOM %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
// COM: get current stake generation power
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        if (block.GetBlockTime() + nStakeMinAge2 > txNew.nTime - nMaxStakeSearchInterval) //PALM
    ...
    // Calculate coin age reward
    {
        ...
        nCredit += GetProofOfStakeReward(nCoinAge, nBits, txNew.nTime, pindexPrev->nHeight + 1);
		printf("CreateCoinStake : block index %d\n", pindexPrev->nHeight + 1);
    }
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64& nBalanceInQuestion, int& nOrphansFound, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %sCOM %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %sCOM %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), hash.ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
